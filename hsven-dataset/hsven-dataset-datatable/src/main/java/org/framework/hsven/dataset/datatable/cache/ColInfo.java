...
public class ColInfo implements Serializable
{
	...
	public String name=null;
 	...
 	{
 		if (name==null){ throw new DataSetException("name can not be set to null." + name );}
		if (name.trim().length()==0){ throw new DataSetException("name can not be set to empty." + name );}
 		if (title==null){ throw new DataSetException("title can not be set to null." + title );}
		if (title.trim().length()==0){ throw new DataSetException("title can not be set to empty." + title );} 		
 		if(format!=null)
 		{
 			if (format.trim().length()==0){ throw new DataSetException("format can not be set to empty." + format );}
 		}
 	}
